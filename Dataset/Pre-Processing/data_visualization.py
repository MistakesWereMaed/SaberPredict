...
def draw_random_action(df, n, seed=42):
    groups = (
    ...
    sampled_groups = groups.sample(n, random_state=seed)
    for _, row in sampled_groups.iterrows():
        plot_poses(df, row["start_idx"], row["end_idx"], 10)

def draw_random_window(df, n, seed=42):
    groups = (
    ...
    sampled_groups = groups.sample(n, random_state=seed)
    for _, row in sampled_groups.iterrows():
        plot_poses(df, row["start_idx"], row["end_idx"], 4)